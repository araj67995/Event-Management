...
function openUpcomingPopup(clickedButton) {
  ...
  document.getElementById("popup-event-name").textContent =
    cells[0].textContent;
  document.getElementById("popup-event-type").textContent =
    cells[1].textContent;
  document.getElementById("popup-event-location").textContent =
    cells[2].textContent;
  document.getElementById("popup-event-date").textContent =
    cells[3].textContent;
  document.getElementById("popup-event-visibility").textContent =
    row.dataset.visibility || "Not set";
  document.getElementById("popup-event-ticket").textContent =
    row.dataset.ticket || "Not set";
  document.getElementById("popup-event-details").textContent =
    row.dataset.details || "No details available";
...
function openPastPopup(clickedButton) {
  ...
  popup.querySelector("#popup-event-location").textContent =
    cells[2].textContent;
  popup.querySelector("#popup-event-date").textContent = cells[3].textContent;
  popup.querySelector("#popup-event-visibility").textContent =
    row.dataset.visibility || "Not set";
  popup.querySelector("#popup-event-ticket").textContent =
    row.dataset.ticket || "Not set";
  popup.querySelector("#popup-event-details").textContent =
    row.dataset.details || "No details available";
  popup.querySelector("#popup-event-collected-amount").textContent =
    row.dataset.collectedAmount || cells[4].textContent || "Not set";
  popup.querySelector("#popup-evnet-vender-expense").textContent =
    row.dataset.venderExpense || "Not set";
  popup.querySelector("#popup-event-venue-cost").textContent =
    row.dataset.venueCost || "Not set";
  popup.querySelector("#popup-evnet-ticket-sale").textContent =
    row.dataset.ticketSale || "Not set";
...
function showUpcomingAction(actionType) {
  hideUpcomingActionPanels();
  const panelId =
    actionType === "cancel" ? "cancel-event-panel" : "complete-event-panel";
...
function openBookingPopup(clickedButton) {
  ...
  document.getElementById("booking-event-name").value =
    row.dataset.eventName || "";
  document.getElementById("booking-event-type").value =
    row.dataset.eventType || "";
  document.getElementById("booking-event-location").value =
    row.dataset.eventLocation || "";
  document.getElementById("booking-ticket-price").value =
    row.dataset.ticketPrice || "";
  document.getElementById("booking-total-cost").value =
    row.dataset.totalCost || "";
  document.getElementById("booking-event-details").value =
    row.dataset.details || "";

  setCheckedRadio("booking-visibility", row.dataset.visibility);
  setCheckedRadio("booking-ticket", row.dataset.ticket);
  updateTicketPriceField(
    "booking-ticket",
    "booking-ticket-price-field",
...
setupTicketPriceToggle(
  "ticket",
  "quick-ticket-price-field",
  "quick-ticket-price",
);
setupTicketPriceToggle(
  "booking-ticket",
...
function updateRevenueDashboard() {
  ...
  const metrics =
    monthlyRevenueData[selectedMonth] || monthlyRevenueData.default;
  const totalExpense =
    metrics.vendorExpense +
    metrics.venueCost +
    metrics.staffCost +
    metrics.marketingCost;
  const due = metrics.totalRevenue - metrics.collectedAmount;
  const profit =
    metrics.collectedAmount + metrics.ticketSales - totalExpense - metrics.loss;
  const averageEventValue = metrics.totalEvents
    ? metrics.totalRevenue / metrics.totalEvents
    : 0;
  ...
  document.getElementById("cancelled-events").textContent =
    metrics.cancelledEvents;
  document.getElementById("total-revenue").textContent = formatLakh(
    metrics.totalRevenue,
  );
  document.getElementById("collected-amount").textContent = formatLakh(
    metrics.collectedAmount,
  );
  document.getElementById("vendor-expense").textContent = formatLakh(
    metrics.vendorExpense,
  );
  document.getElementById("venue-cost").textContent = formatLakh(
    metrics.venueCost,
  );
  document.getElementById("ticket-sales").textContent = formatLakh(
    metrics.ticketSales,
  );
  document.getElementById("average-event-value").textContent =
    formatLakh(averageEventValue);
  ...
  document.getElementById("profit-note").textContent =
    profit >= 0 ? "After expenses" : "Needs attention";
  document.getElementById("chart-month-label").textContent = selectedMonth
    ? `Finance overview for ${selectedMonth}`
...
function drawRevenueChart(metrics) {
  ...
  const barWidth = Math.max(
    28,
    (chartWidth - barGap * (chartItems.length - 1)) / chartItems.length,
  );
